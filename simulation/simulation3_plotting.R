...
#par(mar = mar1)
{
  pdf(file = "./sim3.pdf", width = 12, height = 5) 
  #par(mfrow = c(2,3))
  ...
  for(num in 1:length(nums)){
    ...
    mean_mat = apply(results_tmp[,,,1], c(1,3), mean) #1: prediction
    upper_mat = apply(results_tmp[,,,1], c(1,3), function(a) quantile(a, probs = .975)) #1: prediction
    lower_mat = apply(results_tmp[,,,1], c(1,3), function(a) quantile(a, probs = .025)) #1: prediction
  ...
  for(num in 1:length(nums)){
    results_tmp = results[num,,,,]
    mean_mat = apply(log(results_tmp[,,,4]), c(1,3), mean) #5: time
    upper_mat = apply(log(results_tmp[,,,4]), c(1,3), function(a) quantile(a, probs = .975)) 
    lower_mat = apply(log(results_tmp[,,,4]), c(1,3), function(a) quantile(a, probs = .025)) 
