...
Example.result.all = array(NA,dim=c(length(nums),length(budgets),nsim,3,5))
obj_list<-c()
...
for (j in 1:(length(nums)) ){
  for (k in 1:(length(budgets)) ){
    ...
    for(sim in 1:nsim){ 
      ...
      a<-roc(Y.test, as.vector(X.test%*%FCR.result$beta), quiet = TRUE)
      ...
      a<-roc(Y.test, as.vector(X.test%*%HCR.result$beta), quiet = TRUE)
      ...
      a<-roc(Y.test, as.vector(X.test%*%beta.lasso), quiet = TRUE)
      auc.lasso = as.numeric(a$auc)
      FPR.lasso=sum(beta==0&beta.lasso!=0)/sum(beta==0)
      FNR.lasso=sum(beta!=0&beta.lasso==0)/sum(beta!=0)
      
      ## Report Results
      Example.result=rbind(c(Prediction.FCR, auc.FCR,  FPR.FCR, FNR.FCR),
                            c(Prediction.HCR, auc.HCR, FPR.HCR, FNR.HCR),
                            c(Prediction.lasso, auc.lasso, FPR.lasso, FNR.lasso))
