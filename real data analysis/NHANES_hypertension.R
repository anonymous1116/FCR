...
p = dim(X)[2]
results_hypertension = array(NA,dim=c(length(budget_seq),nsim,3,4)) 
...
for (sim in 1:nsim){
  for (j in 1:length(budget_seq) ){
    ...
    results_hypertension[j,sim,,] = rbind(c(pred.FCR, logl.FCR, auc.FCR, time.FCR), 
...
results = results_hypertension
tmp = list(group.costs, budget_seq, results, obj_results)
save(tmp,file="revision_results/results_hypertension.RData")
