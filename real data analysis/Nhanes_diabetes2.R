...
p = dim(X)[2]
results_diabetes2 = array(NA,dim=c(length(budget_seq),nsim,2,4)) 
...
for (sim in 1:nsim){
  for (j in 1:length(budget_seq) ){
    budget = budget_seq[j]
    ind = sample(1:dim(X)[1], size = floor(dim(X)[1] * 0.5))
    train_y = y[ind]
    train_X = X[ind,]
    ...
    results_diabetes2[j,sim,,] = rbind(c(pred.FCR, logl.FCR, auc.FCR, time.FCR), 
...
results = results_diabetes2
tmp = list(group.costs, budget_seq, results, obj_results)
#save(tmp,file="results_diabetes2.RData")
