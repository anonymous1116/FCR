...
results_heart = array(NA,dim=c(length(budget_seq),nsim,3,4))
maxiter = 1000
...
for (j in 1:length(budget_seq) ){
  ...
  for (sim in 1:nsim){
    ...
    ptm=proc.time()[3]
    HCR.result = HCR.logistic.group(train_X,train_y,group,group.costs,budget,maxiter=1000,tol,lambda,alpha,rep(0,p))
    ...
    fresults_heart[j,sim,,] = rbind(c(pred.FCR, logl.FCR, auc.FCR, time.FCR), 
...
#save(tmp,file="revision_results/results_heart.RData")
#load("revision_results/results_heart.RData")
